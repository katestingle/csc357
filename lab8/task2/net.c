...
int create_service(short port)
{
   ...
   // get the socket file descriptor - TCP stream socket
   if ((fd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
   {
      return -1;
   }

   // allow reuse of the port if the service is restarted
   if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR,&yes, sizeof(int)) == -1)
   {
      return -1;
   }  
        
   // bind the socket to the port by associating the socket with a local address and port
   local_addr.sin_family = AF_INET;         /* host byte order */
   local_addr.sin_port = htons(port);       /* convert short to network byte order */
